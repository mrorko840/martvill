  ...
  <div class="col-sm-12" id="shop-add-container">
    <div class="card">
      <div class="card-header">
        <h5><a href="{{ isset($vendor) && !empty($vendor) ? route('vendors.edit', ['id' => $vendor]) : route('shop.index') }}">{{ __('Shops') }}</a> >> {{ __('Add :x', ['x' => __('Shop')]) }}</h5>
      </div>
        ...
        <div class="form-tabs">
          ...
          <div class="tab-content" id="myTabContent">
            <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
              <form action="{{ route('shop.store', ['vendor' => isset($vendor) && !empty($vendor) ? $vendor : null]) }}" method="post" id="shopStore" class="form-horizontal">
                @csrf
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-6">
                      <select class="form-control select2" id="vendor_id" name="vendor_id">
                        ...
                        <option value="{{ $vendor->id }}" {{ old('vendor_id') == $vendor->id || (isset($vendor_id) && $vendor_id == $vendor->id)  ? 'selected' : '' }}>{{ $vendor->name }}</option>
                        @endforeach
                      </select>
                    </div>
                </div>
                ...
                <div class="col-sm-8 px-0">
                  ...
                  <a href="{{ request('vendor_id') ? route('vendors.edit', ['id' => request('vendor_id')]) : route('shop.index') }}" class="btn btn-info custom-btn-small">{{ __('Cancel') }}</a>
                </div>
              </form>
            </div>
          </div>
