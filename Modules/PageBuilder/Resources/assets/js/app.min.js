"use strict";localStorage.removeItem("gjsProject");var escapeName=a=>`${a}`.trim().replace(/([^a-z0-9\w-:/]+)/gi,"-"),editor=grapesjs.init({container:"#gjs",height:"98vh",StorageManager:!1,selectorManager:{escapeName},fromElement:!0,assetManager:{multiUpload:!0,params:{_token:csrfToken},upload:uploadImage,uploadName:"files",assets:images},plugins:["grapesjs-preset-webpage","gjs-blocks-basic","grapesjs-tabs","grapesjs-parser-postcss","gjs-plugin-ckeditor","grapesjs-custom-code","grapesjs-tailwind","grapesjs-plugin-forms"],pluginsOpts:{"grapesjs-tabs":{tabsBlock:{category:"Extra"}},"gjs-preset-webpage":{modalImportTitle:jsLang("Import Template"),modalImportLabel:"<div style=\"margin-bottom: 10px; font-size: 13px;\">"+jsLang("Paste here your HTML/CSS and click Import")+"</div>",modalImportContent:function(a){return a.getHtml()+"<style>"+a.getCss()+"</style>"},textCleanCanvas:jsLang("Are you sure to clear the canvas?")},"gjs-plugin-ckeditor":{options:{language:"en",toolbarGroups:[{name:"styles",groups:["styles"]},{name:"colors",groups:["colors"]},{name:"tools",groups:["tools"]},{name:"others",groups:["others"]},{name:"basicstyles",groups:["basicstyles","cleanup"]},"/",{name:"paragraph",groups:["list","indent","blocks","align","bidi","paragraph"]},{name:"clipboard",groups:["clipboard","undo"]},{name:"editing",groups:["find","selection","spellchecker","editing"]},"/",{name:"links",groups:["links"]},{name:"insert",groups:["insert"]}],removeButtons:"NewPage"}},"grapesjs-custom-code":{},"grapejs-tailwind":{},"grapesjs-plugin-forms":{}}});const processStorePage=a=>{saveButton.set({className:"fa fa-floppy-o icon-blank page-save-button",label:""}),(a.status="info")?triggerNotification(a.message):triggerNotification(a.message)},savePage=function(){saveButton.set({className:"page-save-button",label:`  <svg width="20" height="20" viewBox="0 0 16 16" color="#FFFFFF" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <animateTransform
            ...
            dur="1300ms"
          />
          </svg>`}),ajaxRequest({html:editor.getHtml(),css:editor.getCss(),_token:csrfToken},pageStoreUrl,processStorePage)},redirectToPage=()=>{window.open(pagePreviewUrl,"_blank")};editor.Panels.addButton("options",[{id:"save",className:"fa fa-floppy-o page-save-button",command:savePage,attributes:{title:jsLang("Save Template")}},{id:"redirect",className:"fa fa-arrow-circle-o-right",command:redirectToPage,attributes:{title:jsLang("Preview in new tab")}}]);const saveButton=editor.Panels.getButton("options","save"),ajaxRequest=(a,b,c)=>{$.post(b,a,c)},assetManager=editor.AssetManager;editor.Panels.removeButton("options","export-template"),editor.Panels.getButton("options","gjs-open-import-webpage").set({label:"",className:"fa fa-upload"}),editor.on("asset:upload:response",a=>{if(a.error!==void 0)throw"Failed to upload image.";else assetManager.add(a)});const triggerNotification=a=>{$(".notification-msg-bar").find(".notification-msg").html(a),$(".notification-msg-bar").removeClass("smoothly-hide"),setTimeout(()=>{$(".notification-msg-bar").addClass("smoothly-hide"),$(".notification-msg-bar").find(".notification-msg").html("")},1500)};