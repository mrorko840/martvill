...
@if ($outOfStock['isApprove'])
    <div>
        <div class="border rev-img rounded-md relative">
            <div style="height: {{ empty($component->card_height) ? $isEnableProduct['height'] : $component->card_height }}px" class="h-48 flex justify-center items-center">
                @php
                    ...
                    <div class="absolute left-2.5 top-2.5">
                        @if ($outOfStock['outOfStockVisibility'] == 1)
                            @php $outStock = true @endphp
                                <p class="bg-pinks-2 relative w-max z-10 h-4 text-reds-3 mb-2.5 px-1.5 flex items-center rounded-sm leading-3 roboto-medium font-medium pt-2p text-8 whitespace-nowrap">{{ __('Stock Out') }}</p>
                            @endif
                            @if ($newArrival && $outStock == false)
                                <p class="semi-primary-bg-color h-4 primary-text-color px-1.5 flex items-center rounded-sm relative z-10 mb-2.5 pt-2p leading-3 roboto-medium font-medium text-8 whitespace-nowrap">
                                    {{ __('New Arrival') }}</p>
                            @endif
                            @if (isset($item->featured) && $outStock == false && $newArrival == false)
                                <p class="primary-bg-color relative z-10 h-18p w-max justify-center text-white px-2 flex items-center rounded-sm mb-2.5 leading-3 roboto-medium font-medium text-xss whitespace-nowrap">
                                    {{ __('Featured') }}</p>
                            @endif
                            @if ($item->review_average == 5 && $outStock == false && $newArrival == false)
                                <div class="flex items-center relative z-10 w-max px-1.5 whitespace-nowrap mb-2.5 bg-green-5 h-18p rounded-sm">
                            ...
                            @if ($offerFlag && !$item->isVariableProduct() && $outStock == false && $newArrival == false)
                                <p class="primary-bg-color h-4 w-max relative text-gray-12 mb-2.5 px-2 py-1 flex items-center justify-center rounded-sm leading-3 roboto-medium font-medium z-10 text-8 whitespace-nowrap uppercase">{{ formatCurrencyAmount($item->getDiscountAmount()) }}% {{ __('off') }}</p>
                            @endif
                    </div>
                @endif
            ...
            <div
                ...
                <div class="absolute flex justify-center h-6 cursor-pointer top-15p right-15p">
                    <a href="javascript:void(0)" class="relative block w-fill">
                        <div slot="icon" class="relative left-2">
                            {{-- Add to cart --}}
                            @if (!$item->isVariableProduct() && $outStock == false && !$item->isExternalProduct() && isset($isEnableProduct['add_to_cart']) && $isEnableProduct['add_to_cart'] == 1 && !$item->isGroupedProduct())
                                <a href="javascript:void(0)" class="add-to-cart" data-itemCode={{ $item->code }}>
                                    <div
                                        class="h-6 w-6 p-1 mb-2 text-gray-12 primary-bg-hover border border-gray-2 rounded-full bg-white">
                            ...
                            @if (preference('compare') && isset($isEnableProduct['compare']) && $isEnableProduct['compare'] == 1)
                                <div data-itemId="{{ $item->id }}"
                                    class="h-6 w-6 p-1 absolute mb-2 text-gray-12 bg-white primary-bg-hover border border-gray-2 rounded-full compare-bg {{ isCompared($item->id) ? 'compare-remove' : 'add-to-compare' }} ">
                ...
                @if (isset($isEnableProduct['quick_view']) && $isEnableProduct['quick_view'] == 1 && !$item->isGroupedProduct())
                    <button class="open-view-modal" data-itemCode="{{ $item->code }}">
                        <p class="text-gray-12 font-medium absolute inset-x-0 bottom-0 rounded rounded-t-none text-center text-11 primary-bg-color">
                            {{ __('Quick View') }}</p>
                    </button>
                @elseif (isset($isEnableProduct['quick_view']) && $isEnableProduct['quick_view'] == 1)
                    <a href="{{ route('site.productDetails', ['slug' => $item->slug]) }}">
                        <p class="text-gray-12 font-medium absolute inset-x-0 bottom-0 text-center text-11 primary-bg-color">
                            {{ __('View') }}</p>
                    </a>
                @endif
            </div>
        ...
        <div class="sm:text-center flex flex-col">
                                    ...
                                    <li class="mt-5p pr-2">
                                        <svg class="h-3 w-3" viewBox="0 0 142 142" fill="none"
                                            ...
                                            <path
                                                ...
                                                fill="#C4C4C4" />
                                            <mask id="mask0_2170_1814" class="mask-type-alpha" maskUnits="userSpaceOnUse"
                                                x="3" y="0" width="136" height="129">
                                                <path
                                                    d="M71 0L86.9405 49.0598H138.525L96.7923 79.3804L112.733 128.44L71 98.1196L29.2672 128.44L45.2077 79.3804L3.47499 49.0598H55.0595L71 0Z"
                                                    fill="#C4C4C4" />
                                            </mask>
            ...
            @if (isset($isEnableProduct['price']) && $isEnableProduct['price'] == 1)
                ...
                @else
                    ...
                    @endif
                    <p
                        class="mt-1 md:mt-0 {{ $offerFlag ? 'text-11 font-medium line-through text-gray-10 pl-1 mt-0.5' : 'text-13 md:text-sm text-gray-12 dm-sans font-medium' }}">
