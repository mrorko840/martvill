...
@section('logo', asset(config('instamojo.logo')))

@section('gateway', config('instamojo.name'))
...
@section('content')
    ...
    <form class="pay-form needs-validation"
        action="{{ route('gateway.complete', withOldQueryIntegrity(['gateway' => config('instamojo.alias')])) }}"
        ...
        <button type="submit" class="pay-button sub-btn">{{ __('Pay With Instamojo') }}</button>
    </form>
@endsection
